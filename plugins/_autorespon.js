    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? 'Sakura さくら NEXT PROJECT NOT ACTIVE' : banned ? 'TELAH DI BANNED' : 'Sakura さくら NEXT PROJECT IN HERE',
                '©Sakura さくら',
...
`.trim(), '©Sakura さくら', 'CHAT ME TO BUY', ',owner', { contextInfo: { mentionedJid: [global.owner[0] + '@s.whatsapp.net'] } })
    }
...
SakuraBOT IS THE BEST`).catch(_ => _)
